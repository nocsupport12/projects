...
export function BottomBar({
  ...
  let roomMembersArr = [...roomMembers]
  ...
  useEffect(()=>{
    socket.on("force:stop:screenshare",()=>{
      toggleScreenShare()
      return () => {
        socket.off("force:stop:screenshare",()=>{
          toggleScreenShare()})
      }
    })
  },[socket])
    useEffect(()=>{
      if (delay) {        
        setScreenSharing(localScreenShareOn) 
...
} else {return (<div className="text-white">{recordingState === Constants.recordingEvents.RECORDING_STARTED ? "Host Recording": ""}</div>)}
      ...
      <MobileIconButton
        ...
        onClick={() => {
          if (allowed) {
            ...
            alert('Someone is still sharing their screen')
          }
        }}
        disabled={
         !allowed?isMobile?false:true:true
        }
      />
    ) : (
      <OutlinedButton
