...
import { deleteFundRequestDB } from "../../../../Api/FundRequestApi";
import {
  Box,
  Button,
  Flex,
...
export const DeleteFundRequestModal = ({
  ...
  const deleteEmployeeDB = async () => {
    try {
      const response = await deleteFundRequestDB({ _id: deleteFundRequest });
      if (response) {
        // REALTIME
        // Assuming allUsers is your state array
        let newAllFundRequests = [
          ...allFundRequests.filter((el) => el._id !== deleteFundRequest),
        ]; // Copying the state array
        setAllFundRequests([...newAllFundRequests]);
        setDeleteFundRequest(null);
      }
    } catch (error) {
      console.log(error);
    }
      ...
      <div className="bg-gray-800 text-white shadow-2xl min-w-[300px] min-h-[200px] h-[40vh] w-[30vw] rounded-2xl overflow-auto flex flex-col items-center justify-center">
        ...
        <Flex justify="end" h="10%" w="100%">
          <Button
            ...
            onClick={() => setDeleteFundRequest(null)}
          >
            <IoMdClose />
          </Button>
        </Flex>
