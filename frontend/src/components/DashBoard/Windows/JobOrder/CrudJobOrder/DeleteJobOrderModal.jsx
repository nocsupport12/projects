...
import { deleteJobOrderDB } from "../../../../Api/JobOrderApi";
export const DeleteJobOrderModal = ({
  setDeleteJobOrder,
  deleteJobOrder,
  allJobOrders,
  setAllJobOrders,
  ...
  // DELETE JOBORDER DATABASE
  const deleteJobOrderAPI = async () => {
    try {
      const response = await deleteJobOrderDB({ _id: deleteJobOrder });
      if (response) {
        // REALTIME
        // Assuming allUsers is your state array
        let newJobOrders = [
          ...allJobOrders.filter((el) => el._id !== deleteJobOrder),
        ]; // Copying the state array
        setAllJobOrders([...newJobOrders]);
        setDeleteJobOrder(null);
      }
    } catch (error) {
      console.log(error);
    }
      ...
      <div className="bg-gray-800 text-white shadow-2xl min-w-[300px] min-h-[200px] h-[40vh] w-[30vw] rounded-2xl overflow-auto flex flex-col items-center justify-center">
        ...
        <Flex justify="end" h="10%" w="100%">
          <Button
            ...
            onClick={() => setDeleteJobOrder(null)}
          >
            <IoMdClose />
          </Button>
        </Flex>
